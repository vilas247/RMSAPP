 ...
 * Class SettingsViews
 *
 * Represents a helper class to connect RETAIL MERCHANT & BigCommerce 
...
class SettingsViews
{
	...
	{		
		...
		if(!empty($email) && !empty($accessToken) && !empty($storeHash)){
			...
			if (count($result) > 0) {
				...
				$builderupdate = $db->table('rms_token_validation'); 
				...
				$builderinsert = $db->table('rms_token_validation'); 
	...
	public static function deleteScripts($sellerdb,$acess_token,$store_hash,$email_id,$validation_id){
		...
		if (count($result) > 0) {
			foreach($result as $k=>$v){
				...
				$builderdelete = $db->table('rms_scripts'); 
	...
	public static function injectPaymentScripts($sellerdb,$acess_token,$store_hash,$email_id,$validation_id){
		...
		$url[] = getenv('bigcommerceapp.RMS_SDK');
				...
				if (strpos($response->getHeader('content-type'), 'application/json') != false){
					...
					if(!empty($response)){
						...
						if(isset($response['data']['uuid'])){
							...
							$builderinsert = $db->table('rms_scripts'); 
							$builderinsert->insert($data);
							$rStatus++;
						}
					}
