 ...
 * Class Settings
 *
 * Permit to connect RETAIL MERCHANT with Bigcommerce and vice-versa.
		...
		if($this->request->getMethod() == "post"){
			...
			if(!empty($clientDetails)){
				...
				if (count($result) > 0) {
					...
					$builderupdate = $db->table('custom_rmspay_button'); 
					$builderupdate->where('email_id', $clientDetails['email_id']); 
					...
					$builderinsert = $db->table('custom_rmspay_button'); 
